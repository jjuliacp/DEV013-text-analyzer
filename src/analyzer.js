const analyzer = {  
  ...
  // console.log(text.split(' '));
   let textArray = text.trim().split(' '); // arreglo divide el texto usando el espacio como divisor
  ...
  //console.log(text.trim().replace(/[^\w\s]/g, '').replace(/\s+/g, '').split('').length);
    let characterExcluding =text.trim().replace(/[^\w\s]/g, '').replace(/\s+/g, '').split(''); //
    let count =0;
  ...
  //encontrar los numeros y decimales // (/\b\d+\.?\d+\b/g)//  \b =numero de digitos que esten rodeados por caracteres diferentes (limite de palabra)
  // contar los numeros 
  let count =0;
  let number= text.match(/\b\d+\.?\d*\b/g); //encuentra los numeros y los .decimales 
  if(number){
  ...
  getNumberSum: (text) => {  
    ...
    let number=text.match(/\b\d+\.?\d*\b/g); // encuentra los digitos de los numeros
  let suma =0;
  if (number){
